# ============================================================
# ESB — Minimal Loader (District-level-data.xlsx) 
# Goal: clean, tiny, and ready to analyze
# ============================================================

# ---- Packages (install once if needed) ----
# install.packages(c("tidyverse","readxl","janitor","stringr","readr"))
...
# ---- 0) Config ----
DIST_XLSX <- "District-level-data.xlsx"        # <- your extracted, small Excel
OUT_RDS   <- "esb_df_clean.rds"                # cached clean object (fastest to reuse)
OUT_CSV   <- "esb_df_clean.csv"                # optional for portability

stopifnot(file.exists(DIST_XLSX))

# ---- 1) Helpers (tiny & robust) ----
to_num <- function(x) { 
...
# ---- 2) Read Excel (as text for speed/consistency) & clean names ----
dist_raw <- read_excel(DIST_XLSX, col_types = "text") %>% clean_names()

# ---- 3) Build target (committed) & tidy, typed analysis frame ----
# Columns below follow your charter/research design families: 
# Funding/Policy, Demographics/Economics, Health/EJ, Geography/Operations, plus outcome.

esb_df <- dist_raw %>%
  mutate(
    # Target (robust): prefer explicit flag, fallback to count>0
...
# ---- 4) Save a small, clean copy for later manipulation ----
saveRDS(esb_df, OUT_RDS)
readr::write_csv(esb_df, OUT_CSV)

# ---- 5) Quick confirmation prints (fast, optional) ----
cat("\nSaved:\n -", OUT_RDS, "\n -", OUT_CSV, "\n")
