...
ggplot(pred_locale_df,
  ...
  geom_text(
    ...
    aes(label = paste0(label, ": ", round(weighted_mean * 100, 2), "%"),
        y = weighted_mean * 100, color = as.factor(prioritized_any)),
    x = 4.1, hjust = 1, vjust = -0.6, size = 4.2, fontface = "bold"
  ) +
...
# ===============================================================
# PARAMETRIC SCENARIO SIMULATION & CONFIDENCE INTERVALS
# ===============================================================
B_param <- 400
set.seed(123)
...
# helper: rescale policy allocation to preserve same national prioritization rate
rescale_policy_to_overall <- function(policy_tbl, locale_sizes, target_overall) {
  ...
  tmp %>% mutate(target_rate_adj = pmin(pmax(target_rate * k0, 0), 1)) %>%
...
# convert to lifetime savings (USD) and CI bounds
...
# ===============================================================
# PLOT 3 — Total Lifetime Savings by Policy Scenario
# ===============================================================
...
# ===============================================================
# PLOT 4 — Δ Lifetime Savings vs Current Policy
# ===============================================================
delta_results <- tribble(
  ~scenario, ~mean, ~low, ~high,
  "Rural-focused", mean(sav_rur - sav_curr),
  quantile(sav_rur - sav_curr, .025), quantile(sav_rur - sav_curr, .975),
  "Balanced", mean(sav_bal - sav_curr),
  quantile(sav_bal - sav_curr, .025), quantile(sav_bal - sav_curr, .975),
  "Urban-focused", mean(sav_urb - sav_curr),
  quantile(sav_urb - sav_curr, .025), quantile(sav_urb - sav_curr, .975)
) %>%
  mutate(
    scenario = factor(scenario,
